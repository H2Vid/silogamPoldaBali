...
(function ($) {
  ...
  $("#datepicker,#datepicker2,#datepicker3,#datepicker4,#datepicker5,#datepicker6,#datepicker7").daterangepicker({
    dateFormat: "d MM yy",
    duration: "medium",
    changeMonth: true,
    changeYear: true,
  ...
  (from = $('input[name="date-range-from"]')
    ...
    .on("change", function () {
      to.daterangepicker("option", "minDate", getDate(this));
    })),
  (to = $('input[name="date-range-to"]')
    ...
    .on("change", function () {
      from.daterangepicker("option", "maxDate", getDate(this));
    }));

  function getDate(element) {
    var date;
    try {
      date = $.daterangepicker.parseDate(dateFormat, element.value);
    } catch (error) {
      date = null;
    }
  ...
  // Date Picker
  $(".date-picker__calendar").daterangepicker();
  ...
  function compareDates(startDate, endDate, format) {
    ...
    try {
      dateStart = $.daterangepicker.parseDate(format, startDate);
      dateEnd = $.daterangepicker.parseDate(format, endDate);
  ...
  $.fn.dateRangePicker = function (options) {
    ...
    function storePreviousDateRange(dateText, dateFormat) {
      ...
      if (dateText.length > 0) {
        start = $.daterangepicker.parseDate(dateFormat, dateText[0]);
        if (dateText.length > 1) {
          end = $.daterangepicker.parseDate(dateFormat, dateText[1]);
        }
        lastDateRange = {
          start: start,
          end: end
    ...
    options.beforeShow = function (input, inst) {
      var dateFormat = myDateRangeTarget.daterangepicker("option", "dateFormat");
    ...
    return this.each(function () {
      if (myDateRangeTarget.is("input")) {
        myDateRangeTarget.daterangepicker(options);
      }
  ...
  /** Password Toggler */
  $(document).on('click', '.pass-toggler', function() {
    currentType = $(this).prev('input').attr('type');
    ...
    $(this).prev('input').attr('type', changeTo);
  });

  /** Ajax Priority URL */
  $(document).on('click', '.ajax-priority', function(e) {
    e.preventDefault();

    if ($(this).closest('.sidebar_nav').length > 0) {
      $('.sidebar_nav').find('a.active').removeClass('active');
      $(this).addClass('active');
    }
    ...
    if ($(window).width() < 1150 && (target != '#' && target != '')) {
      // sidebarCollapse();

      $('.overlay-dark-sidebar').removeClass('show');
      $('.sidebar').addClass('collapsed');
      $('.sidebar').addClass('sidebar-collapse');
      $('.contents').addClass('expanded');
    }    

    pageLoadAjax(target, method);
  });

  $('body').on('click', function(e) {
    if (e.target.className == 'dropdown-custom') {
      $(this).toggleClass('active');
    } else if ($(e.target).parents('.dropdown-custom').length > 0) {
      $(e.target).parents('.dropdown-custom').toggleClass('active');
    } else {
...
function pageLoadAjax(target, method)
{
  if (target == '#' || target == '') {
    return;
  }
...
function showLoading()
{
  $(".loader-overlay").show();
  $("#overlayer").show(); 
  $('body').addClass('overlayScroll').removeClass('loaded');
}

function hideLoading()
{
  $(".loader-overlay").fadeOut(100);
  $("#overlayer").fadeOut(100, function () {
    $('body').removeClass('overlayScroll').addClass('loaded');
  });
}