<div class="custom-input" data-plugin="file" {{ isset($value) ? 'has-file' : '' }}  {!! arrayToHtmlProp($attr ?? []) !!}>
    ...
    <input type="file" class="file-metadata-controller form-control" accept="{{ isset($attr['accept']) ? $attr['accept'] : 'file/*' }}" data-path="{{ isset($path) ? encrypt($path) : null }}" data-ajax="{{ route('cms.api.upload-file') }}">
